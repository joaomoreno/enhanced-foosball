	...
	def update(self, red, blue):
		...
		if red == self.nextRed and blue == self.nextBlue:
			if self.nextCount < 10:
				...
				self.red = red
				self.blue = blue		
				self.executor.submit(self.submitScore, red, blue)
		else:
			self.nextRed = red
			self.nextBlue = blue
			self.nextCount = 0
	
	def submitScore(self, red, blue):
		print(red, blue)
		msg = 'red = %d, blue = %d' % (red, blue)
		requests.post('https://67a1cb4d38d1.ngrok.io/api/game/update', json = { 'ConversationId': self.id, 'Message': msg })
